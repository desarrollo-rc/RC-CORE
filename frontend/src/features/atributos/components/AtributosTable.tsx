...
export function AtributosTable({ records, onEdit, onDeactivate, onActivate }: AtributosTableProps) {
    const rows = records.map((record) => (
        <Table.Tr key={record.id_atributo}>
    ...
    return (
        <Table striped highlightOnHover withTableBorder>
            <Table.Thead>
                <Table.Tr>
                    <Table.Th>Nombre del Atributo</Table.Th>
                    <Table.Th>Estado</Table.Th>
                    <Table.Th style={{ textAlign: 'right' }}>Acciones</Table.Th>
                </Table.Tr>
            </Table.Thead>
            <Table.Tbody>
                {rows.length > 0 ? rows : <Table.Tr><Table.Td colSpan={3}><Text c="dimmed" ta="center">No se encontraron atributos</Text></Table.Td></Table.Tr>}
            </Table.Tbody>
        </Table>
    );
}